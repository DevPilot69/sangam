/**
 * Split payroll entry breakdown JSON into earnings vs deduction lines for payslip UI/PDF.
 */
...
export function splitPayslipBreakdown(
  ...
  if (
    ...
    const deductions = normalizeSection(o.deductions, 'Deductions', deductionsTotal);
...
function normalizeSection(
  ...
  if (!section || typeof section !== 'object' || Array.isArray(section)) {
    if (fallbackAmount > 0) return [{ label: fallbackLabel, amount: fallbackAmount }];
    return [];
  }
