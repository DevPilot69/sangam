...
export class ReportsService {
  ...
  async run(tenantId: string, dto: RunReportDto): Promise<ReportResult> {
    const def = getReportDefinition(dto.reportId);
    if (!def) throw new NotFoundException('Report not found');

    switch (dto.reportId) {
      case 'payroll_register':
        return this.payrollRegister(tenantId, dto.filters);
      case 'payroll_department_summary':
        return this.payrollDepartmentSummary(tenantId, dto.filters);
      case 'bank_disbursement':
        return this.bankDisbursement(tenantId, dto.filters);
      case 'statutory_summary':
        return this.statutorySummary(tenantId, dto.filters);
      case 'attendance_summary':
        return this.attendanceSummary(tenantId, dto.filters);
      case 'leave_utilization':
        return this.leaveUtilization(tenantId, dto.filters);
      case 'employee_master':
        return this.employeeMaster(tenantId, dto.filters);
      default:
        throw new BadRequestException('Unsupported report');
    }
  }

  private periodFromFilters(filters: Record<string, string | number>) {
    ...
    if (!periodYear || !periodMonth || periodMonth < 1 || periodMonth > 12) {
      throw new BadRequestException('periodYear and periodMonth are required');
    }
    return { periodYear, periodMonth };
  }

  private async findRunForPeriod(
    tenantId: string,
    periodYear: number,
    periodMonth: number,
  ) {
  ...
  private async payrollRegister(
    tenantId: string,
    filters: Record<string, string | number>,
  ): Promise<ReportResult> {
    const { periodYear, periodMonth } = this.periodFromFilters(filters);
    const run = await this.findRunForPeriod(tenantId, periodYear, periodMonth);
    ...
    if (!run?.entries.length) {
      return {
        ...
        columns,
        rows: [],
        summary: { message: 'No locked payroll entries for this period' } as never,
      };
    }
  ...
  private async payrollDepartmentSummary(
    tenantId: string,
    filters: Record<string, string | number>,
  ): Promise<ReportResult> {
    const reg = await this.payrollRegister(tenantId, filters);
    const byDept = new Map<string, { count: number; net: number; gross: number }>();
  ...
  private async bankDisbursement(
    tenantId: string,
    filters: Record<string, string | number>,
  ): Promise<ReportResult> {
    const { periodYear, periodMonth } = this.periodFromFilters(filters);
  ...
  private async statutorySummary(
    tenantId: string,
    filters: Record<string, string | number>,
  ): Promise<ReportResult> {
    const reg = await this.payrollRegister(tenantId, filters);
  ...
  private async leaveUtilization(
    tenantId: string,
    filters: Record<string, string | number>,
  ): Promise<ReportResult> {
  ...
  private async employeeMaster(
    ...
    return {
      ...
      rows: employees.map((e) => ({
        ...
        bankReady:
          e.bankName && e.bankAccount && e.ifsc ? 'Yes' : 'No',
        pan: e.pan ?? '—',
      })),
