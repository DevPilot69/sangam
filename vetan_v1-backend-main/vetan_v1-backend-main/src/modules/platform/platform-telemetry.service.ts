...
import { deriveLiveStatus } from './platform-tenant.utils';
...
export class PlatformTelemetryService {
  ...
  async getSummary() {
    ...
    const billingAgg = await this.prisma.tenantBillingOps.aggregate({
      _sum: { monthlyFeeInr: true, monthlyServerCostInr: true },
      _count: true,
    });
    const overdueCount = await this.prisma.tenantBillingOps.count({
      where: { paymentStatus: 'OVERDUE' },
    });
    const unpaidCount = await this.prisma.tenantBillingOps.count({
      where: { paymentStatus: 'UNPAID' },
    });
    const paidCount = await this.prisma.tenantBillingOps.count({
    ...
    const recentTenants = await this.prisma.tenant.findMany({
      ...
      select: {
        ...
        billingOps: {
          select: { paymentStatus: true, monthlyFeeInr: true, monthlyServerCostInr: true },
        },
  ...
  async listTenants(opts?: { search?: string; limit?: number }) {
    ...
    return tenants.map((t) => this.platformTenants.mapTenantListRow(t));
  }
  ...
  async getTenant(id: string) {
    ...
    const lastPayroll = await this.prisma.payrollRun.findFirst({
      where: { tenantId: id },
      orderBy: [{ periodYear: 'desc' }, { periodMonth: 'desc' }],
    });
    ...
    const legalDocuments = await this.prisma.tenantLegalDocument.findMany({
      where: { tenantId: id },
      orderBy: { createdAt: 'desc' },
      select: {
        id: true,
    ...
    const paymentDocuments = await this.financialDocs.listPaymentDocuments(id);
    ...
    return {
      ...
      subscription: tenant.subscriptions[0]
        ? {
            ...
            razorpayConfigured: !!tenant.subscriptions[0].razorpaySubscriptionId,
          }
        : null,
